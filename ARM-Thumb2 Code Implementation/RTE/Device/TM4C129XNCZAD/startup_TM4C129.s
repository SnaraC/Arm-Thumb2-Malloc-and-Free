...
Reset_Handler   PROC
                ...
                IMPORT  SystemInit
                IMPORT  __main
				IMPORT  _systemcall_table_init ; directive to the name file not in the current assembly call to use system table init
				IMPORT  _heap_init ; similar to above line but call to use heap_init
				; set up MSP, do not change the following three lines
				LDR     R0, =__initial_sp ; thread mode uses MSP
				MSR     MSP, R0 ; move status from register to MSP(master stack pointer)
				ISB  ;Instruction Synchronization Barrier, flushes the pipeline of the processor.
				
                LDR     R0, =SystemInit ; call the systeminit so that function in svc is invoked
                BLX     R0				; jump using R0 register
				LDR     R0, =_heap_init ; call the heap_init so that function heap initialize in svc is invoked to jump to heap.s
				BLX     R0				; jump using R0 register
				LDR     R0, =_systemcall_table_init ; ldr systemcall table init to R0 and jump using below line so that function system table init is invoked to heap.s
				BLX     R0				; jump using R0 register
				
				; set up thread stack, do not change any code from here to ENDP
				LDR     R0, =__initial_user_sp ; thread mode uses user stack (user mode)
				MSR     PSP, R0 		; move status from register above initial above line to PSP (Process Stack Pointer)
...
SVC_Handler     PROC
                ...
                MRS	R1, PSP						; run in PSP mode (Process stack pointer)
				STR	R0, [R1]					; store value in R1
